...
Player.prototype.Init = function()
{
	...
	// SURVIVORS MOD: { [fromPlayerId]: total resources tributed to us }.
	this.tributesReceivedFrom = {};
...
/**
 * SURVIVORS MOD.
 * @return {number} - The player ID that should inherit this player's units on
 *     defeat, or 0 (Gaia) when there is no eligible ally or the setting is off.
 */
Player.prototype.GetSurvivorsHeir = function()
{
	// Default to enabled when the attribute is absent (e.g. older saved games).
	const enabled = !InitAttributes || !InitAttributes.settings ||
	...
	const allies = cmpDiplomacy.GetAllies();
	if (!allies)
		return 0;

	const activeAllies = allies.filter(ally => {
		if (ally === this.playerID || ally === 0)
			return false;
		const cmpAlly = QueryPlayerIDInterface(ally);
		return cmpAlly && cmpAlly.IsActive() && !cmpAlly.IsRemoved();
	});

	if (!activeAllies.length)
		return 0;

	// Prefer the ally who tributed us the most; ties and "nobody tributed"
	// fall back to the first active ally in diplomacy order.
	let heir = activeAllies[0];
	let bestTribute = 0;
	for (const ally of activeAllies)
	{
		const tributed = this.tributesReceivedFrom[ally] || 0;
		if (tributed > bestTribute)
		{
			bestTribute = tributed;
			heir = ally;
		}
	}
...
Player.prototype.SetState = function(newState, message)
{
	...
	else
	{
		...
		// SURVIVORS MOD: units go to a surviving ally instead of Gaia. Structures
		// still go to Gaia, so the defeated player's buildings are not gifted.
		const heir = this.GetSurvivorsHeir();
		const newOwner = entity => {
			if (!heir)
				return 0;
			const cmpIdentity = Engine.QueryInterface(entity, IID_Identity);
			return cmpIdentity && cmpIdentity.HasClass("Unit") ? heir : 0;
		};
		...
		for (const entity of entities)
		{
			...
			cmpOwnership.SetOwnerQuiet(newOwner(entity));
		}
...
/**
 * SURVIVORS MOD: track how much each ally has tributed to us, so that on defeat
 * our units go to the most generous surviving ally.
 */
Player.prototype.OnGlobalTributeExchanged = function(msg)
{
	...
	const total = Object.values(msg.amounts).reduce((sum, amount) => sum + amount, 0);
