...
Player.prototype.GetDisplayedColor = function () {
  return this.displayDiplomacyColor
    ? Engine.QueryInterface(this.entity, IID_Diplomacy).GetColor()
    : this.color;
};
...
Player.prototype.RefundResources = function (amounts) {
  const cmpStatisticsTracker = QueryPlayerIDInterface(
    this.playerID,
    IID_StatisticsTracker,
  );
  if (cmpStatisticsTracker)
...
Player.prototype.SetState = function (newState, message) {
  ...
  if (won) cmpRangeManager.SetLosRevealAll(this.playerID, true);
    ...
    const allies = QueryPlayerIDInterface(
      this.playerID,
      IID_Diplomacy,
    ).GetAllies();
    ...
    for (let entity of entities) {
      ...
      if (entityIdentity.HasClass("Unit")) {
        cmpOwnership.SetOwnerQuiet(allyId);
      } else {
    ...
    for (let entity of entities)
      Engine.PostMessage(entity, MT_OwnershipChanged, {
        entity: entity,
        from: this.playerID,
        to: Engine.QueryInterface(entity, IID_Identity).HasClass("Unit")
          ? allyId
          : 0,
      });
  }
...
Player.prototype.TributeResource = function (player, amounts) {
  ...
  if (!this.IsActive() || !cmpPlayer.IsActive()) return;
...
Player.prototype.OnGlobalPlayerDefeated = function (msg) {
  ...
  const soundGroup = cmpSound.GetSoundGroup(
    this.playerID === msg.playerId
      ? "defeated"
      : Engine.QueryInterface(this.entity, IID_Diplomacy).IsAlly(msg.playerId)
        ? "defeated_ally"
        : this.HasWon()
          ? "won"
          : "defeated_enemy",
